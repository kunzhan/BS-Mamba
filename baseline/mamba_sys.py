...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=4, num_classes=2, depths=[2, 2, 9, 2], 
                 dims=[12, 24, 48, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
...
if __name__ == "__main__":
    ...
    model = VSSM().to('cuda')
    int = torch.randn(4,3,384,384).cuda()
    out = model(int)
    print(out.shape)
