...
from baseline.multi_mamba import MultiScan
...
class VSSM(nn.Module):
    ...
    #         #q-shift操作计算x*
    #         xx = q_shift(x, shift_pixel=1, gamma=1/4, patch_resolution=None)
    ...
    def forward(self, x):
        ...
        skip_list[0] = skip_list[0].permute(0, 3, 1, 2)
        skip_list[1] = skip_list[1].permute(0, 3, 1, 2)
        skip_list[2] = skip_list[2].permute(0, 3, 1, 2)
        skip_list[3] = skip_list[3].permute(0, 3, 1, 2)
        for i, o in enumerate(skip_list): # 4 倍上采样
            skip_list[i] = F.interpolate(o, scale_factor=4, mode='bilinear') # skip_list[0] [2, 64, 64, 96]   [3]  [2, 8, 8, 768]        
